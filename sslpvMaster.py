...
class SSLPVinst(master.StochIPinst):
		...
		for s in range(self.Nscen):
			...
			for j in range(self.Nfv):
				if j < self.m:
					...
					x_scen[j] = self.scensub[s].addVar(lb=0,ub=1,obj=objCoef[j],name="x"+str(j))
			y = {}
			for i in range(self.Nn):
				for j in range(self.m):
					#y[i,j] = self.scensub[s].addVar(vtype=GRB.BINARY,obj=-regCoefy*self.qMtr[i,j],name="y_"+str(i)+","+str(j))
					y[i,j] = self.scensub[s].addVar(lb=0,ub=1,obj=-regCoefy*self.qMtr[i,j],name="y_"+str(i)+","+str(j))
			y0 = {}
			for j in range(self.m):
				y0[j] = self.scensub[s].addVar(lb=0.0,obj=regCoefy*self.q0Vec[j],name="y0_"+str(j))
			for j in range(self.m):
				self.scensub[s].addConstr(x_scen[j+self.m]<=x_scen[j])
				self.scensub[s].addConstr(quicksum(self.dMtr[i,j]*y[i,j] for i in range(self.Nn))-y0[j] <= self.Nu*(1-self.rho)*x_scen[j]+self.Nu*self.rho*x_scen[j+self.m])
			for i in range(self.Nn):
				self.scensub[s].addConstr(quicksum(y[i,j] for j in range(self.m)) == self.hMtr[i,s])
		...
		for s in range(self.Nscen):
			...
			for i in range(self.Nn):
				for j in range(self.m):
					...
					y[i,j] = self.scenrc[s].addVar(lb=0,ub=1,obj=-self.qMtr[i,j],name="y_"+str(i)+","+str(j))
			y0 = {}
			for j in range(self.m):
			...
			for i in range(self.Nn):
				self.scenrc[s].addConstr(quicksum(y[i,j] for j in range(self.m)) == self.hMtr[i,s])
	...
	def SolveExtensive(self):
		...
		for j in range(self.m):
			...
			x_con[j] = Extensive.addVar(lb=0,ub=1,obj=self.cVec[j+self.m])
		for s in range(self.Nscen):
			for i in range(self.Nn):
				for j in range(self.m):
					y[s,i,j] = Extensive.addVar(lb=0.0,ub=1.0,obj=-self.pVec[s]*self.qMtr[i,j])
		for s in range(self.Nscen):
			for j in range(self.m):
				y0[s,j] = Extensive.addVar(lb=0.0,obj=self.pVec[s]*self.q0Vec[j])
		for j in range(self.m):
			Extensive.addConstr(x_con[j]<=x[j])
		for s in range(self.Nscen):
			for j in range(self.m):
				Extensive.addConstr(quicksum(self.dMtr[i,j]*y[s,i,j] for i in range(self.Nn))-y0[s,j] <= (1-self.rho)*self.Nu*x[j]+self.rho*self.Nu*x_con[j])
		for s in range(self.Nscen):
			for i in range(self.Nn):
				Extensive.addConstr(quicksum(y[s,i,j] for j in range(self.m)) == self.hMtr[i,s])
	...
	def BuildPrimal(self):
		...
		for i in range(self.m):
			self.PrimalMaster.addConstr(self.x[i+self.m]<=self.x[i])
	...
	def readData(self):
		...
		self.Nscen = fline[2]
		# Read cVec
		self.cVec = np.append((1-self.rho)*self._readVec(x[1], np.float32),self.rho*self._readVec(x[1], np.float32))
		# Read qMtr
	...
	def SolveBendersSub(self,scen_id,x_input):
		...
		for j in range(self.m):
			benderssub.getConstrByName("xconstr"+str(j)).RHS = self.Nu*((1-self.rho)*x_input[j]+self.rho*x_input[j+self.m])
		benderssub.update()
		benderssub.optimize()
		...
		for j in range(self.Nfv):
			if j < self.m:
				subg[j] = self.Nu*(1-self.rho)*benderssub.getConstrByName("xconstr"+str(j)).pi
			else:
				subg[j] = self.Nu*self.rho*benderssub.getConstrByName("xconstr"+str(j-self.m)).pi
	...
	def SolveScenSub(self,scen_id,objCoef,regCoefy,tlimit,MIPgap='nan'):
		...
		yObjV = -sum(self.qMtr[i,j]*self.scensub[scen_id].getVarByName("y_"+str(i)+","+str(j)).x for i in range(self.Nn) for j in range(self.m))+\
			sum(self.q0Vec[j]*self.scensub[scen_id].getVarByName("y0_"+str(j)).x for j in range(self.m))
		...
		for k in range(self.scensub[scen_id].SolCount-1):
			...
			for j in range(self.Nfv):
				soln[j] = modelSoln[j]
			soln[self.Nfv]=-sum(self.qMtr[i,j]*modelSoln[self.Nfv+i*self.m+j] for i in range(self.Nn) for j in range(self.m))+\
				sum(self.q0Vec[j]*modelSoln[self.Nfv+self.Nn*self.m+j] for j in range(self.m))
			SubOpt.append(soln)
	...
	def SolveRecourse(self,scen_id,x_input):
		for j in range(self.m):
			self.scenrc[scen_id].getConstrByName("xconstr"+str(j)).RHS = self.Nu*(1-self.rho)*x_input[j]+self.Nu*self.rho*x_input[self.m+j]
	...
	def BendersBnC(self,timeLimit=60*60,tol=1e-6,cutTime=0,methodName='BnC'):
		...
		ScenLB = {}
		for s in range(self.Nscen):
			ScenLB[s] = -sum(max(self.qMtr[i,j] for j in range(self.m))*self.hMtr[i,s] for i in range(self.Nn))
		'''
		for i in range(self.m):
